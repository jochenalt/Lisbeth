...
StatePlanner::StatePlanner() :
		dt_(0.0), h_ref_(0.0), n_steps_(0), RPY_(Vector3::Zero())
{
}
...
void StatePlanner::initialize(Params &params)
{
	...
	dt_vector_ = VectorN::LinSpaced(n_steps_, dt_,
			static_cast<double>(n_steps_) * dt_);
}
...
void StatePlanner::computeReferenceStates(Vector6 const &q, Vector6 const &v,	Vector6 const &vref)
{
	...
	for (int i = 0; i < n_steps_; i++)
	{
		if (vref(5) != 0)
		{
			referenceStates_(0, 1 + i) = (vref(0)
					* std::sin(vref(5) * dt_vector_(i))
					+ vref(1) * (std::cos(vref(5) * dt_vector_(i)) - 1.0)) / vref(5);
			referenceStates_(1, 1 + i) = (vref(1)
					* std::sin(vref(5) * dt_vector_(i))
					- vref(0) * (std::cos(vref(5) * dt_vector_(i)) - 1.0)) / vref(5);
		}
		else
		{
		...
		referenceStates_(6, 1 + i) = vref(0)
				* std::cos(referenceStates_(5, 1 + i))
				- vref(1) * std::sin(referenceStates_(5, 1 + i));
		referenceStates_(7, 1 + i) = vref(0)
				* std::sin(referenceStates_(5, 1 + i))
